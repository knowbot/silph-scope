...
public class Gen4PkmnParser : APkmnParser
{
    ...
    private const int BattleStatsSize = 100;
    ...
    public override Pkmn? Parse(ReadOnlySpan<byte> pkmnData, bool isParty)
    {
        ...
        Span<byte> blocks = stackalloc byte[EncryptedSize];
        pkmnData.Slice(0x8, EncryptedSize).CopyTo(blocks);
        ...
        if (!isDecrypted)
        {
            Decrypt(blocks, checksum);
        }
        // TODO: decide what to do if check fails again
        if (!IsValidData(blockA))
            throw new ParserException($"Invalid Pokémon data, read species #{blockA.Read<ushort>()}.");
        ...
        if (isParty)
        {
            Span<byte> battleStats = stackalloc byte[BattleStatsSize];
            pkmnData.Slice(0x88, BattleStatsSize).CopyTo(battleStats);
            if (!isDecrypted)
                Decrypt(battleStats, pId);
    ...
    {
        return UnencryptedSize + EncryptedSize + BattleStatsSize;
    }

    public override int GetBoxPkmnSize()
    {
        return UnencryptedSize + EncryptedSize;
    }
}
